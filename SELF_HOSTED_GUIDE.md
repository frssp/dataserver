...
This file is auto-generated by `docker/entrypoint.sh` from `docker/config.inc.php.template`, using values from the `php-fpm.environment` section of `docker/docker-compose.yml`. The relevant env vars are:

| Env var | Purpose |
|---------|---------|
| `ZOTERO_BASE_URL` | URL clients reach this server on. Zotero 7+ opens this URL during sign-in, so it must be reachable from client machines. |
...
| `ZOTERO_AUTH_SALT` | Password hashing salt — **change in production** and never rotate (would break all stored passwords) |
...
> The browser URL is built from `ZOTERO_BASE_URL` (set in `docker/docker-compose.yml`). It must be reachable from the client machine — `localhost` only works if the client runs on the same host as the server. See section 2.
