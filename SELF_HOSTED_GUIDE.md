...
This file is auto-generated by `docker/entrypoint.sh` from `docker/config.inc.php.template`, using values from `docker/.env` (which `docker compose` auto-loads when run from the `docker/` directory). Copy the example to start:

```bash
cp docker/.env.example docker/.env
# then edit docker/.env with your deployment values
```
...
| Env var | Purpose |
|---------|---------|
| `ZOTERO_BASE_URL` | URL clients reach this server on. Zotero 7+ opens this URL during sign-in, so it must be reachable from client machines (no `localhost` unless the client runs on this host). Trailing slash required. |
| `ZOTERO_API_SUPER_USERNAME` | Super-user (HTTP Basic) for `admin.php` |
| `ZOTERO_API_SUPER_PASSWORD` | Super-user password — **change in production** |
| `ZOTERO_AUTH_SALT` | Password hashing salt — **change in production** and never rotate (would break all stored passwords). Generate with `openssl rand -hex 32`. |
| `MYSQL_ROOT_PASSWORD` | MySQL root (only used for `docker compose exec mysql -uroot`) |

**Changing a value after first boot:**

```bash
vi docker/.env                            # update the value
rm include/config/config.inc.php          # remove the cached generated copy
docker compose -f docker/docker-compose.yml restart php-fpm   # entrypoint regenerates
```

> ⚠ **Super-password drift:** `admin.php` lets you change the super-password from the web UI, but it only rewrites `include/config/config.inc.php` in place — it does **not** update `docker/.env`. If you regenerate (as above) after a web-UI change, the password silently reverts to whatever is in `.env`. Workaround: whenever you change it via the web, immediately mirror the new value into `docker/.env`.

If you need a different shape (more services, custom hosts), pre-create `include/config/config.inc.php` before the first container start and the entrypoint will leave it alone. Reference shape:
...
Go to **Settings → Sync** and click **Sign In**.

Zotero 7+ opens a browser to `http://YOUR_SERVER:8080/login?session=<token>`. Enter:

- **Username:** your username (e.g., `testuser`)
- **Password:** your password (e.g., `test123`)

Once you see "Signed in", return to Zotero — it will pick up the new API key automatically (within ~5 seconds).

> The browser URL is built from `ZOTERO_BASE_URL` (set in `docker/.env`). It must be reachable from the client machine — `localhost` only works if the client runs on the same host as the server. See section 2.
...
Also update `ZOTERO_BASE_URL` in `docker/.env` to match (e.g. `http://YOUR_SERVER_IP:8080/`) so the sign-in flow opens the right URL, then `rm include/config/config.inc.php && docker compose restart php-fpm` to apply.
