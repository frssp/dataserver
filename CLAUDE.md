...
- **Build output**: `htdocs/library/` — **tracked in git** (committed), generated by `npm run build`. After changing `web-library/` source you MUST rebuild and commit the regenerated `htdocs/library/` bundle, or the deployed SPA will be stale (it's served straight from the repo, no build step on deploy).
...
npm run build    # Production build to htdocs/library/ (tracked — commit the output)
```

> **Important:** `htdocs/library/` is committed to the repo and served as-is by
> nginx — there is no build step on deploy. Whenever you edit `web-library/`
> source, run `npm run build` and commit the regenerated `htdocs/library/`
> files in the same change, otherwise the live SPA won't reflect your edits.
...
| Static web assets (`htdocs/library/**`, images, css)    | **None.** Served directly by nginx. These files are tracked in git. |
| `web-library/` source                                   | **None** for `npm run dev`. For prod: `npm run build`, then **commit the regenerated `htdocs/library/` bundle** (it's tracked and served as-is — no build on deploy). No restart. |
